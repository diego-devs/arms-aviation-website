...
const Fleet: React.FC<FleetProps> = ({ onNavigate }) => {
    ...
    const prevSlide = useCallback(() => {
        ...
        const newIndex = isFirstSlide ? fleetData.length - 1 : currentIndex - 1;
        setCurrentIndex(newIndex);
    }, [currentIndex, fleetData.length]);

    const nextSlide = useCallback(() => {
        const isLastSlide = currentIndex === fleetData.length - 1;
        const newIndex = isLastSlide ? 0 : currentIndex + 1;
        setCurrentIndex(newIndex);
    }, [currentIndex, fleetData.length]);

    const handleSelectAircraft = () => {
        onNavigate('quote', fleetData[currentIndex].id);
    };
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="mt-12 relative max-w-5xl mx-auto">
                    <div className="relative h-[34rem] w-full overflow-hidden rounded-lg shadow-2xl">
                        {/* Image Slides */}
                        {fleetData.map((aircraft: any, index: number) => (
                    ...
                    <div className="flex justify-center mt-6 space-x-2">
                        {fleetData.map((_: any, index: number) => (
